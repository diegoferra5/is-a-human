...
def test_auto_resolve_from_repo_root(hackmty_paths):
    resolved = resolve_dataset_paths()
    assert resolved.root.resolve() == hackmty_paths.root.resolve()
    assert resolved.audio_dir.resolve() == hackmty_paths.audio_dir.resolve()
